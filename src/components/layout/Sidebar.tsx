...
import {
  LayoutDashboard, Database, Monitor, FileSignature,
  BoxSelect, ScanLine, Calculator, Settings, ShieldAlert,
} from 'lucide-react'
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const navLink = (href: string, key: string, icon: React.ReactNode, label: string, badge?: number) => {
  ...
  return (
    ...
    `}>
      ...
      <div className="h-16 flex items-center px-6 bg-slate-950 border-b border-slate-800">
        ...
        <span className="text-lg font-bold text-white tracking-wide">AssetCop MVP</span>
      </div>
      ...
      <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
        ...
        {!isEmployee && (
          <>
            <p className="text-xs font-semibold text-slate-500 uppercase mt-8 mb-3 tracking-wider">현장 및 재무 관리</p>
            <nav className="space-y-1">
              {navLink('/map', 'map',
              ...
              {canManageSystem && navLink('/depreciation', 'depreciation',
        ...
        {canManageSystem && (
          <>
            <p className="text-xs font-semibold text-slate-500 uppercase mt-8 mb-3 tracking-wider">시스템 설정</p>
            <nav className="space-y-1">
              {navLink('/master', 'master',
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/50">
        <div className="flex items-center text-xs text-slate-400">
          ...
          <span>현재 권한: <strong className="text-white">{currentUser.role.toUpperCase()}</strong></span>
        </div>
      </div>
    </aside>
    </>
